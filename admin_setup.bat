...
:: Luego escribe en el archivo de log la configuración de red usando las varaibles obtenidas anteriormente
echo [%DATE% %TIME%] Configuracion red: IP=%IP%, MASK=%MASCARA%, GATEWAY=%PUERTA% >> log_admin.txt

:: Esta línea registra en el mismo archivo una operación de copia realizada,
:: utilizando las rutas de origen y destino definidas en las variables ORIGEN y DESTINO.
echo [%DATE% %TIME%] Copia realizada de "%ORIGEN%" a "%DESTINO%" >> log_admin.txt

:: Informa al usuario que la operación ha terminado y sugiere revisar el archivo de log.
echo Operacion completada. Revisa el archivo log_admin.txt
:: Pausa la ejecución del script hasta que el usuario presione una tecla.
pause